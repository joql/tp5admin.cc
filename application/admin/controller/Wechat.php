...
class Wechat extends Common{
    ...
    protected $wx_config;//微信公众号配置
    protected $app;//微信实例化app
    /**
     * 微信账户设置
     */
    public function initialize()
    {
        ...
        $this->wx_config = json_decode($value,true);
        //实例化easywechat类
        $config = [
   ...
   function getInstanceWchatMenu($instance_id)
    ...
    public function updatemenutoweixin(){
        ...
        if (! empty($auth_info['value']['appid']) && ! empty($auth_info['value']['appsecret'])) {
            $res = $this->app->menu->create($result);
            if (! empty($res)) {
                if ($res['errcode'] == 0) {
                    $retval = 1;
                } else {
    ...
    public function updateInstanceMenuToWeixin($instance_id)
    {
        ...
        //$menu_array = json_encode($menu_array['button'],JSON_UNESCAPED_UNICODE);
        return $menu_array['button'];
    }
